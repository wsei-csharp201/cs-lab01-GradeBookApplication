...
- [ ] Add support for Ranked Grading
	...
	- [D] Override `RankedGradeBook`'s `GetLetterGrade` method
	...
	- [D] Override `RankedGradeBook`'s `CalculateStudentStatistics` method
		- [ ] Short circuit the method if there are less than 5 students.
			- If there are less than 5 students write "Ranked grading requires at least 5 students." to the Console.
	...
	- [D] Update `StartingUserInterface`'s `CreateCommand` method
		- [D] Update `CreateCommand`'s Conditions
		...
		- [D] return a new GradeBook based on the provided type
	...
	- [D] Update `StartingUserInterfaces`'s `HelpCommand` method
	...
	- [D] Make the `BaseGradeBook` class abstract
		- [ ] Add the `abstract` keyword to the `BaseGradeBook` declaration.

- [ ] Add support for weighted GPAs
	- [D] Add `IsWeighted` property to `BaseGradeBook`
